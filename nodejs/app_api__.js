...
//json読み込み
var fs = require('fs');
var gen_stu = JSON.parse(fs.readFileSync('gen_student.json', 'utf8'));
...
function calc_reward(quest,id){
  ...
  for(var i=0;i<que_data.quests[quest.quest_num].combination.length;i++){
    if(que_data.quests[quest.quest_num].combination[i].toString()==spe_list.toString()){
      ...
      if(quest.quest_num==1){if(check==false){great_data.q1[i].push(id);} great_reward_money=1000000*(1/(great_data.q1[i].length-1));}
      if(quest.quest_num==2){if(check==false){great_data.q2[i].push(id);} great_reward_money=5000000*(1/(great_data.q2[i].length-1));}
      if(quest.quest_num==3){if(check==false){great_data.q3[i].push(id);} great_reward_money=5000000*(1/(great_data.q3[i].length-1));}
      if(quest.quest_num==4){if(check==false){great_data.q4[i].push(id);} great_reward_money=5000000*(1/(great_data.q4[i].length-1));}
      if(quest.quest_num==5){if(check==false){great_data.q5[i].push(id);} great_reward_money=10000000*(1/(great_data.q5[i].length-1));}
          ...
          if(quest_lists[i].state==0){
            ...
            for(var j=0;j<quest_lists[i].users.length;j++){
              if(quest_lists[i].users[j].id==req.body.uid){
                quest_lists[i].users.splice(i,1);
...
// (POST http://localhost:8080/api/add_stu)
    .post(function(req, res) {
      ...
      if(rare==true){
        ...
        add_name=gen_stu.gen[add_type].name;
        add_skill=gen_stu.gen[add_type].skill;
        add_personality=6;
      }else{
        add_type=Math.floor( Math.random() * 2 );
        add_name=gen_stu.gen[add_type].name[Math.floor( Math.random() * 50 )];
      ...
      connection.query('insert into student_M set ?',
                       ...
                       logic:Math.ceil(gen_stu.gen[add_type].status[0]*((req.body.level/10)+1)),
                       develop:Math.ceil(gen_stu.gen[add_type].status[1]*((req.body.level/10)+1)),
                       communicate:Math.ceil(gen_stu.gen[add_type].status[2]*((req.body.level/10)+1)),
                       skill1:add_skill,
                       skill2:0,
                       skill3:0,
        ...
        function(error,results,fields){
          ...
          var sql2 = "SELECT student1,student2,student3,student4,student5,student6 FROM user_M where cast(uid as unsigned)='"+req.body.uid+"';";
          //console.log(sql2);
          var query2 = connection.query(sql2);
          query2.on('result', function(rows2) {
            for(var i=1;i<=6;i++){
          ...
          //console.log(gen_stu.gen[add_type].info);
          ...
          res.jsonp({
            ...
            "status":[Math.ceil(gen_stu.gen[add_type].status[0]*((req.body.level/10)+1)),Math.ceil(gen_stu.gen[add_type].status[1]*((req.body.level/10)+1)),Math.ceil(gen_stu.gen[add_type].status[2]*((req.body.level/10)+1))], //random
            "pic":gen_stu.gen[add_type].pic, //idより
            "course_t":new Date().getTime(),
            "collabo":0,
            "stu_type":add_type, //random?
            "info":gen_stu.gen[add_type].info //生徒info
        });
      });
...
// (POST http://localhost:8080/api/join_event)
    .post(function(req, res) {
      ...
      if(regi_n_==-2){ //削除
        delete participants_list[req.body.uid];
        ret_message={"regi_n":regi_n_,"sid":sid_};
      }else if(regi_n_==-3){ //login時確認
        var sid_=0;
        for(var i in participants_list){
          if(i==req.body.uid){
        ...
        if(found==false){
          //追加
          participants_list[req.body.uid]={"uname":req.body.uname,"regi_n":req.body.regi_n,"sid":req.body.sid,"sname":req.body.sname,"status":req.body.status};
        }else{
          if(regi_n_==-1){
            //確認
...
// (POST http://localhost:8080/api/join_event)
    .post(function(req, res) {
      //console.log(req.body.uid);
      //console.log(req.body.sid);
      if(req.body.eventnum==0){
        ret_message={
          "event_name":event_data.event0.event_name,
          "reward_num":event_data.event0.reward_num,
          "reward_money":event_data.event0.reward_money,
        };
      }else if(req.body.eventnum==1){
        ret_message={
          "event_name":event_data.event1.event_name,
          "reward_num":event_data.event1.reward_num,
          "reward_money":event_data.event1.reward_money,
        };
      }else if(req.body.eventnum==2){
        ret_message={
          "event_name":event_data.event2.event_name,
          "reward_num":event_data.event2.reward_num,
          "reward_money":event_data.event2.reward_money,
        };
      }
